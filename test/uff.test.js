...
test('술폭사이드류(S=O + 단일결합 2개)는 S_3+6이 아니라 S_3+2다 (이웃 개수 오판정 회귀)', () => {
  // 메틸 2개(단일) + O(이중): 이웃은 3개뿐이지만 예전엔 "이웃 3개 이상이면 무조건 S_3+6"이라
  // 결합차수 합(4)과 무관하게 SF6급 육배위(theta0=90°) 취급을 받았다. 결합차수 합이 6 미만이면
  // S_3+2(92.1°)여야 한다 — 90°와 92.1°는 숫자 차는 작아 보여도, buildTerms가 theta0=90°일 때
  // n=4 주기 결합각 퍼텐셜을 쓰므로 실제 수렴 형태가 완전히 달라진다(관련 diag: 90°/90° 고정).
  const m = build({
    atoms: [['S', [0, 0, 0]], ['C', [1.82, 0, 0]], ['C', [-0.91, 1.58, 0]], ['O', [-0.91, -1.58, 0]]],
    bonds: [[0, 1], [0, 2], [0, 3, 2]],
  });
  assert.equal(typeAtom(m, 0), 'S_3+2');
});

test('결합차수 합이 실제로 6 이상이면 여전히 S_3+6이다 (설폰류)', () => {
  // S(=O)(=O) + 단일결합 2개: 결합차수 합 2+2+1+1=6.
  const m = build({
    atoms: [['S', [0, 0, 0]], ['C', [1.8, 0, 0]], ['C', [-1.8, 0, 0]],
            ['O', [0, 1.6, 0]], ['O', [0, -1.6, 0]]],
    bonds: [[0, 1], [0, 2], [0, 3, 2], [0, 4, 2]],
  });
  assert.equal(typeAtom(m, 0), 'S_3+6');
});

test('포스핀옥사이드류(P=O + 단일결합 3개)는 결합차수 합 5로 P_3+5다 (이웃 개수 오판정 회귀)', () => {
  // 메틸 3개(단일) + O(이중): 이웃 4개뿐이라 예전 n>=5 기준으로는 P_3+3(93.8°)에 남았지만
  // 결합차수 합은 1+1+1+2=5로 실제 5가 원자가다.
  const m = build({
    atoms: [['P', [0, 0, 0]], ['C', [1.8, 0, 0]], ['C', [-0.9, 1.6, 0]],
            ['C', [-0.9, -1.6, 0]], ['O', [0, 0, 1.8]]],
    bonds: [[0, 1], [0, 2], [0, 3], [0, 4, 2]],
  });
  assert.equal(typeAtom(m, 0), 'P_3+5');
});
