...
**Ch2 의자 미션의 근거:** `cyclohexane_chair` 프리셋에서 C0의 수소 두 개는 인덱스 6·7이고, 좌표상 H7이 고리 평면 밖(+Z)에 있어 axial이다. H7을 C로 바꾸면 axial-메틸사이클로헥세인이 되고, 같은 구조에 `flipZ`를 적용하면 고리가 뒤집혀 그 메틸이 equatorial이 된다. 두 상태는 **원자 목록이 완전히 같아 위상도 같으므로** 총에너지 비교가 🟡로 정당하다. 아래 테스트가 이 axial/equatorial 판정을 실제로 검증한다 — 좌표 읽기가 틀렸다면 거기서 죽는다.
